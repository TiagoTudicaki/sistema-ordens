...
const equipamentoService = {
  async criar(dados) {
    ...
    const camposVazios = validarCamposVazios({
      cliente_id,
      local,
      identificador,
    });
    ...
    if (camposVazios.length > 0) {
      throw new Error(
        `O(s) campo(s) ${camposVazios.join(", ")} são obrigátorio(s)`,
      );
    }
    ...
    if (modelo != null) {
      ...
      if (!modeloValido) {
        throw new Error(
          "Modelo não deve possuir caracteres especiais ou ponto",
        );
      }
    }
    ...
    if (serie != null) {
      ...
      if (!serieValida) {
        throw new Error(
          "Serie não dever possuir caracteres especiais ou ponto",
        );
      }
    }
    ...
    if (tipo_gas != null) {
      ...
      if (!tipo_gasValido) {
        throw new Error(
          "O tipo do gas não deve possuir espaço ou caracteres especiais ou pontos",
        );
      }
    }
    const camposComDados = {};

    const tipoDeEquipamento = [
      "acj",
      "hi-wall",
      "piso-teto",
      "multi-split",
    ...
    if (tipo != null) {
      camposComDados.tipo = validarCampoEnum(tipo, tipoDeEquipamento);
    }
  ...
  async atualizar(id, camposExistentes) {
    ...
    if (camposTrimados.tipo != null) {
      dados.tipo = validarCampoEnum(camposTrimados.tipo, tipoDeEquipamento);
    }
    ...
    if (camposTrimados.capacidade_btu != null) {
      dados.capacidade_btu = padronizarCapacidade(
        camposTrimados.capacidade_btu,
      );
    }
